...
const TopDoctors = () => {
  ...
  const { doctors } = useContext(AppContext); // ✅ now it works

  return (
    <div className="flex flex-col items-center gap-4 py-16 text-gray-800" id="top-doctors">
      ...
      <div className="w-full grid grid-cols-5 gap-4 pt-5 px-3 sm:px-0">
        {doctors?.slice(0, 10).map((item, index) => (
          <div 
            ...
            key={index} 
            className="bg-[#F0F5FF] rounded-lg overflow-hidden shadow-sm text-center border border-gray-200 hover:-translate-y-1 transition-transform duration-300"
          >
            {/* Doctor Image */}
            <img 
              src={item.image} 
